...
const AsyncStoryFnWrapper = ({ storyFn }) => {
  ...
  useEffect(() => {
    ...
    if (story instanceof Promise) {
      story.then(setChildren, (e) => {
        setChildren("storyFn failed.");
...
storiesOf("stateDef", module)
  ...
  .add("useActions + useGetSelector", () => {
    const C = () => {
      ...
      const handler = () =>
        update({ counter: { $set: counter ? counter + 1 : 1 } });
      return (
        <div>
...
storiesOf("Root", module)
  ...
  .add("From WebShare", () => {
    ...
    const nextUrl =
      ...
      `&share_target_text=${encodeURIComponent(sharedUrl)}`;
...
// @spinner example
//
storiesOf("@spinner-container", module).add("Default", () => {
  const C = () => {
...
storiesOf("useLoader", module).add("Default", () => {
  const C = () => {
